...
@st.cache_data
def load_data():
    # [지능형 경로 설정 추가] 이 파일이 있는 위치를 기준으로 잡습니다.
    BASE_PATH = os.path.dirname(os.path.abspath(__file__))

    try:
        # 데이터 파일 이름 앞에 자동으로 경로를 붙여줍니다.
        df_b = pd.read_csv(os.path.join(BASE_PATH, 'blossoms.csv'), encoding='cp949', skiprows=2, header=None)
        ...
        df_w = pd.read_csv(os.path.join(BASE_PATH, 'avgtemp_rain_sun.csv'), encoding='cp949')
...
if not data.empty:
    ...
    with col1:
        ...
        # 선생님의 rf 접두사와 중괄호 수식을 그대로 유지합니다.
    ...
    st.error("데이터 파일을 찾을 수 없습니다. 'blossoms.csv'와 'avgtemp_rain_sun.csv' 파일이 이 코드와 같은 폴더에 있는지 확인해 주세요.")
